...
/// Workgroup size (fixed for simplicity, can make dynamic later)
let bitonicWorkgroupSize: Int = 64

/// Compute shader for bitonic sort operations
/// Handles all 4 step types: flip/disperse × local/global
// Ported from bitonicCompute.ts
let bitonicComputeShader = """
...
// Currently hardcoding workgroup size to 64, but can make dynamic later
var<workgroup> local_data: array<u32, 128>;  // 64 * 2
...
fn computeMain(
    ...
    switch uniforms.algo {
        ...
        case ALGO_LOCAL_FLIP: {
            ...
            let offset = workgroup_id.x * 128u;  // 64 * 2
        ...
        case ALGO_LOCAL_DISPERSE: {
            let offset = workgroup_id.x * 128u;
            local_data[local_id.x * 2u] = input_data[offset + local_id.x * 2u];
            local_data[local_id.x * 2u + 1u] = input_data[offset + local_id.x * 2u + 1u];
